...
public class SList {
    ...
    /** creat a SList from a IntArray.
     **/
    public SList(int[] intArray) {
        
        head=new SListNode(new Integer(intArray[0]));
        SListNode cur=head;
    ...
    public void squish() {
        ...
        SListNode diffNode=head;
        SListNode currentNode=head;
        while(currentNode!=null){
            while(currentNode!=null&&currentNode.item.equals(diffNode.item)){
                currentNode=currentNode.next;
                size--;
            }
            size++;// size再加一，是因为size--；有一次是自己跟自己相等进入循环体了
            diffNode.next=currentNode;
    ...
    public void twin() {
        ...
        while(curNode!=null)
        {
            SListNode dupNode=new SListNode(curNode.item);
            dupNode.next=curNode.next;
            curNode.next=dupNode;
            curNode=dupNode.next;
            size++;//注意不能忘记slist的另一个field->size.
        }
    }
    ...
    /**
     *  test squish()a list of [0 0 0 0 1 1 0 0 0 3 3 3 1 1 0  ].  Make it to [ 0 1 0 3 1 0 ]
     **/
     private static void testSquish() {
         int[] myIntArray={0,0,0,0,1,1,0,0,0,3,3,3,1,1,0};
         SList mySList= new SList(myIntArray);
         System.out.println();
         System.out.println(mySList.toString());
         mySList.squish();
         System.out.println(mySList.toString());

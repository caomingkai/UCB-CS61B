...
//    System.out.println("table.capacity£º  "+table.capacity());
	int collisionNum=0;
	for(int i=0;i<table.capacity();i++){
		System.out.println("| Bucket "+i+" |£º  "+table.bucket()[i].toString());
		if(table.bucket()[i].length()>1){
			collisionNum++;
		}
	}
/*-------------------------------Collision Test---------------------------------------------*/	
	float n= (float)table.size();
	float N= (float)table.capacity();
	int x=(int)(n-N+N*Math.pow((1-1/N),n));

	System.out.println();
	System.out.println("Boards number is: "+numBoards);
//	System.out.println("Boards number is: "+table.size());
	System.out.println("Bucket number is: "+table.capacity());
	System.out.println("the expected number of collisions£º  "+x);
	System.out.println("the actual number of collisions£º  "+collisionNum);

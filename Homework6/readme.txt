                              CS 61B  Homework 6
                      Due 4pm Friday, October 27, 2006
...
The methods you will implement are a subset of those listed on page 389 of
Goodrich and Tamassia (no iterators are used in this assignment), plus a
makeEmpty() method which removes every entry from a hash table.  There are also
two HashTableChained constructors.  One lets applications specify an estimate
of the number of entries that will be stored in the hash table; the other uses
a default size.  Both constructors should create a hash table that uses a prime
number of buckets.  (Several methods for identifying prime numbers were
discussed early in the semester.)  In the first constructor, shoot for a load
factor between 0.5 and 1.  In the second constructor, shoot for around 100
buckets.  Descriptions of all the methods may be found in Dictionary.java and
...
code in a separate "list" package.  There's no need to move the list code into
the "dict" package.)
...
can test it).  insert(), find(), and remove() should all use this
compFunction() method.
...
buckets to different indices.  The better compression function, as I said in
lecture, is
...
dictionary, so we won't have to calculate the value twice.

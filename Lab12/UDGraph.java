...
public class UDGraph
{
     ...
     *    length 2 from v to w in "thisf" graph.
    ...
    public UDGraph paths(int length) {
        ...
        if(length>2){
            UDGraph tempGraph = paths(length-1);//原本path＝length-1，所以只需在此基础上，增加一个点，检查是否连通即可。
            for(int i=0; i< vertices; i++){
                for(int j=0; j< vertices; j++){
                    if( tempGraph.hasEdge(i, j) == true ){//在length－1的基础上寻找连通节点
                        for(int k=0; k< vertices; k++){
                            if( this.hasEdge(j, k) == true ){//this表明只增加1个节点；然而，若用tempGraph则会增加length－1条边，length个点
                                 newGraph.addEdge(i, k);
                            }
                        }
                    }

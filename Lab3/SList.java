...
//        testSlistFrontEnd();
    ...
    private static void testSlistCreation() {
        int[] intArray={6, 9 ,12};
        int intFront=3;
        int intEnd=15;
        int size=0;;
        //  验证对象数组的下标从0还是1开始？——从0开始！
        System.out.println("---------------"+ intArray.length);
        SListNode[] nodeArray= new SListNode[3];
        nodeArray[0]=new SListNode(new Integer(99));
        nodeArray[0].item=new Integer(11);
        nodeArray[1]=new SListNode(new Integer(88));
        nodeArray[1].item=new Integer(22);
        nodeArray[2]=new SListNode(new Integer(77));
        nodeArray[2].item=new Integer(33);
        System.out.println("========"+nodeArray[0].item);
        
        SList mySList=new SList();
        nodeArray[0]=new SListNode(new Integer(intArray[0]));
        mySList.head=nodeArray[0];
        for(int i=1;i<intArray.length;i++){
            nodeArray[i]=new SListNode(new Integer(0));
            nodeArray[i].item=new Integer(intArray[i]);
            nodeArray[i-1].next=nodeArray[i];
            size=i;
        }
        mySList.tail=nodeArray[size];
        mySList.size=size+1;
        //        SList mySList=new SList();
        //        mySList.head=new SListNode(new Integer(intArray[0]));
        //        SListNode cur=mySList.head;
        //        for(int i=1;i<intArray.length;i++){
        //            cur.next=new SListNode(new Integer(intArray[i]));
        //            cur=cur.next;
        //            size=i;
        //        }
        //        mySList.size=size+1;
        
//     把［6 9 12］改成［3 6 9 12 15］

        
    
        System.out.println();
        System.out.println("Here is a list which should be:[6 9 12]. It is : "
                           + mySList.toString());
        System.out.println("Size of myList is 3. It is : "
                           + mySList.size);
        
        mySList.insertFront(new Integer(3));
        mySList.insertEnd(new Integer(15));
        System.out.println();
        System.out.println("Here is a modified list which should be:[3 6 9 12 15]. It is : "
                           + mySList.toString());
        System.out.println("Size of myList is 5. It is : "
                           + mySList.size);
    }
